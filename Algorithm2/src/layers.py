...
class Dense(Layer):
    ...
    def _get_relational_adj_list3(self):
        ...
        def _np_mat2sp_adj3(np_mat, row_pre, col_pre):
            # print('生成矩阵，然后返回的矩阵互为转置矩阵')
            # 6036+6729
            n_all3 = self.n_entity + self.n_user
        ...
        for r_id in self.relation_dict.keys():
            ...
            #K:[[129955*129955矩阵(但是rows∈[23566,129954] cols∈[23566,129953]才有值）]]
            #item-item邻接矩阵
            K, K_inv = _np_mat2sp_adj3(np.array(self.relation_dict[r_id]), row_pre=self.n_item, col_pre=self.n_item)
            #adj_mat_list[??]=[129955*129955矩阵]
...
########1
        ...
        # if True:
            ...
            u_g_embeddings, i_g_embeddings1 = tf.split(all_embeddings1, [self.n_user, self.n_item], 0)
...
########2
        if self.dataset!='music':
            ...
            i_g_embeddings2, e_g_embeddings = tf.split(all_embeddings2, [self.n_item, self.n_entity], 0)
...
####crosse
            v, e = ea_embeddings, e_g_embeddings# shape=(b,16)
            ...
            # [batch_size * dim, dim]
            c_matrix = tf.reshape(c_matrix, [-1, self.input_dim])# shape=(b*16,16)
            c_matrix_transpose = tf.reshape(c_matrix_transpose, [-1, self.input_dim])# shape=(b*16,16)

            # [batch_size, dim]  shape=(b,16)
            v_output = tf.reshape(tf.matmul(c_matrix, self.weight_vv) + tf.matmul(c_matrix_transpose, self.weight_ev),[-1, self.input_dim]) + self.bias_v
            e_output = tf.reshape(tf.matmul(c_matrix, self.weight_ve) + tf.matmul(c_matrix_transpose, self.weight_ee),[-1, self.input_dim]) + self.bias_e


            ea_e=v_output

####crossu
            v, e = ua_embeddings, u_g_embeddings# shape=(b,16)
            ...
            # [batch_size, dim]  shape=(b,16)
            v_output1 = tf.reshape(tf.matmul(c_matrix, self.weight_vv) + tf.matmul(c_matrix_transpose, self.weight_ev),[-1, self.input_dim]) + self.bias_v
            e_output = tf.reshape(tf.matmul(c_matrix, self.weight_ve) + tf.matmul(c_matrix_transpose, self.weight_ee),[-1, self.input_dim]) + self.bias_e


            ua_e=v_output1

        return ua_e, ea_e, lap_list,self.adj_r_list,self.A
